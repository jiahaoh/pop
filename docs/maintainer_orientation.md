...
| Errors and privacy | Errors become `ServiceError`; transport adds provider help links; runtime code contains no logging calls | Bob 1.20.0 host logs captured source text independently of Pop; treat local/exported logs as sensitive and recheck during release smoke tests |
